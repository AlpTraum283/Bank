...
public interface ObjectRepository extends JpaRepository<ObjectDto, Integer> {
    ...
    @Query(value = "select o.obj_id as accountId, o.name as accountName " +
            "from object o " +
    ...
    @Query(value = "select p.value as TYPE, p2.value as SUM, o.date as DATE " +
            "from object o, parameter p, parameter p2 " +
            "where o.owner = :id and o.type = \'" + OBJECT_TYPE_TRANSFER + "\' and p.attr_id = " + OPERATION_ATTRIBUTE_ID + " and p2.attr_id = " + SUM_ATTRIBUTE_ID + " and o.obj_id = p.obj_id and o.obj_id = p2.obj_id", nativeQuery = true)
    List<Map<String,String>> selectOperationsByAccountId(@Param("id") Integer id);
}
